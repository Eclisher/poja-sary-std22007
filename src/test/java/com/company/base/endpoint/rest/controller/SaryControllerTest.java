...
public class SaryControllerTest {
    ...
    void testGetImageUrls() throws IOException {
        ...
        when(bucketComponent.presign("blacks/" + id, Duration.ofMinutes(5))).thenReturn(new URL(originalImageUrl));
        when(bucketComponent.presign("grayscale/" + id + "_bw.jpg", Duration.ofMinutes(5))).thenReturn(new URL(grayscaleImageUrl));
    ...
    void testUploadImageToBucket_ImageDownloadFailure() throws IOException {
        // Given
        String id = "testId";
        MockMultipartFile image = new MockMultipartFile("image", "testImage.jpg", "image/jpeg", "test image content".getBytes());
        String expectedErrorMessage = "Error uploading image to bucket.";
        when(bucketComponent.upload(any(), any())).thenThrow(new RuntimeException(expectedErrorMessage));
